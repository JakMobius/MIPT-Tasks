//
// Created by Темыч on 26.04.2021.
//

#ifndef BONK_COMPILER_REINVENTED_X86_COLORIZER_HPP
#define BONK_COMPILER_REINVENTED_X86_COLORIZER_HPP
...
struct register_colorizer {
    ...
    // by itself
    dynamic_array<e_machine_register> registers_to_release;

    dynamic_array<e_machine_register> registers_released_by_command;

    // Stack of unused registers
    dynamic_array<e_machine_register> unused_registers_stack;

    // Array of abstract registers associated with corresponding each real machine register
    abstract_register *machine_register_map;

    register_colorize_context_state* state;
    command_buffer* source;

    dynamic_array<register_colorize_context_state> state_stack;
    hash_table<jmp_label*, jmp_label*> label_map;

    command_buffer* output;

    int frame_size;
    dynamic_array<int> released_frame_cells;

    bool construct(command_buffer* input);
    ...
    abstract_register get_colorized_register(asm_command* command, abstract_register reg, bool is_write);
    ...
    void locate_register_at_machine_register(abstract_register reg, e_machine_register loc, asm_command* command);
...
#endif //BONK_COMPILER_REINVENTED_X86_COLORIZER_HPP
