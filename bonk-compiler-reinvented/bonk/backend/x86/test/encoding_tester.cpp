...
void test_encodings(FILE* target) {
    command_buffer* buffer = command_buffer::create();

    buffer->root_list->insert_tail(mov_command::create_reg64_reg64(rcx, rdi, buffer->root_list));

//    buffer->root_list->insert_tail(and_command::create_reg8_reg8(rax, rbx, buffer->root_list));
//    buffer->root_list->insert_tail(or_command::create_reg8_reg8(rax, rbx, buffer->root_list));
//
//    buffer->root_list->insert_tail(mov_command::create_reg64_reg64(rax, rbx, buffer->root_list));
//
...
//    for(int i = 0; i < 16; i++) {
//        buffer->root_list->insert_tail(mov_command::create_reg64_imm64(i, 0xBEBEBEBEFAFAFAFA, buffer->root_list));
//    }

    // Check all mov reg, reg
//    for(int i = 0; i < 16; i++) {
//        for(int j = 0; j < 16; j++) {
//            buffer->root_list->insert_tail(mov_command::create_reg64_reg64(i, j, buffer->root_list));
//        }
//    }
...
//    buffer->root_list->insert_tail(xchg_command::create_reg64_reg64(rax, rbx, buffer->root_list));
//    buffer->root_list->insert_tail(xchg_command::create_reg64_mem64(rax, command_parameter_memory::create_reg(rbx), buffer->root_list));
//    buffer->root_list->insert_tail(xchg_command::create_mem64_reg64(command_parameter_memory::create_reg(rbx), rax, buffer->root_list));

    buffer->dump(stdout, 0);
    command_encoder* bytes = buffer->to_bytes();

    fwrite(bytes->point(0), 1, bytes->size, target);
