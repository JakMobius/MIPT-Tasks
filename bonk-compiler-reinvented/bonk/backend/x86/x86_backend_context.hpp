...
#ifndef BONK_COMPILER_REINVENTED_X86_BACKEND_CONTEXT_H
#define BONK_COMPILER_REINVENTED_X86_BACKEND_CONTEXT_H
...
struct backend_context {
    ...
    dynamic_array<backend_context_state> state_stack;
    ...
    static backend_context* create(compiler* compiler, macho::macho_file* target);
    bool construct(compiler* compiler, macho::macho_file* target);
    ...
    general_jmp_command* append_oper_jmp_command(tree_node_operator* oper, bool inversed);
    ...
    general_jmp_command* compile_boolean_jump(tree_node* node, bool check_for);
    ...
    bool compile_logic_operand_recursive(tree_node* node, jmp_label* control_label, operator_type oper_type, bool check_for);
    bool compile_logic_recursive(tree_node_operator* oper, jmp_label* control_label, operator_type oper_type, bool check_for);
    ...
    tree_node* call_argument_list_get_value(tree_node_list<tree_node_call_parameter*>* argument_list,
                                            tree_node_identifier* identifier);

    void locate_procedure_parameter(variable* parameter);

    tree_node* compile_nth_argument(variable_function* func, tree_node_list<tree_node_call_parameter*>* argument_list, int i);
...
#endif //BONK_COMPILER_REINVENTED_X86_BACKEND_CONTEXT_H
