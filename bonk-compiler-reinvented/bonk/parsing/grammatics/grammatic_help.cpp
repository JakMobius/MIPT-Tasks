...
bool parse_grammatic_help(parser* thou, tree_node_list<tree_node*>* target) {
    ...
    if(next->type == BONK_LEXEME_KEYWORD && next->keyword_data.keyword_type == BONK_KEYWORD_HELP) {
        ...
        const char* library_name = next->identifier_data.identifier->variable_name;
        const char* library_extension = ".bs";

        char* full_path = (char*)calloc(strlen(library_storage) + strlen(library_name) + strlen(library_extension) + 1, sizeof(char));
        ...
        dynamic_array<lexeme>* lexemes = thou->linked_compiler->lexical_analyzer->parse_file(full_path, source);

        if(!thou->linked_compiler->state && lexemes) {
            parser* nested_parser = parser::create(thou->linked_compiler);
