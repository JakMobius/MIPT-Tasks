...
int main(int argc, const char* argv[]) {
    ...
    bonk_lexical_analyzer_parse_file(compiler->lexical_analyzer, real_path, file);

    if(!compiler->state) {
        //print_lexemes(compiler->lexical_analyzer->lexemes);
        s_tree* result = bonk_parser_parse_file(compiler->parser);

        if(!compiler->state) {
            //tree_serialize(result, stdout);
