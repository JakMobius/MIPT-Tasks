...
namespace bonk::ede_backend {
    backend_context* backend_context::create(compiler* compiler, FILE* destination) {
    CHECK_ALLOC_CONSTRUCT(backend_context, compiler, destination)
}

bool backend_context::construct(compiler* compiler, FILE* destination) {
    scope_stack_offset = 0;
    processor_stack_usage = 0;
    linked_compiler = compiler;
    if(!scope_stack.construct()) return false;
...
void backend_context::reference_variable(tree_node_identifier* identifier) {
    ...
    if(scope == scope_stack.scopes.get(0)) {
...
void backend_context::compile_math_comparsion(tree_node* node) {
    if(node->type == TREE_NODE_TYPE_OPERATOR) {
        ...
        if(comparing_instruction != nullptr) {
            ...
            fprintf(target, "%s.d _cmp_%lu_ok\n"
                                         "push.d 0\n"
                                         "jmp _cmp_%lu_end\n"
                                         "_cmp_%lu_ok:\n"
                                         "push.d 1\n"
...
void backend_context::compile_call(tree_node_call* call) {
    ...
    if(argument_calculation_offset > 0) {
        fprintf(target, "push.ul (rdx + 0x%lx)\n"
                                     "pop.ul rdx\n",
    ...
    for(int i = 0; i < function_arguments->variables.size; i++) {
        variable* argument = function_arguments->variables.get(i);

        tree_node* value = call_argument_list_get_value(argument_list, argument->identifier);
        if(value == nullptr) {
            parser_position* position = function_name->source_position;
            linked_compiler->error_positioned(position, "'%s' requires contextual variable '%s'", function_name->variable_name, argument->identifier->variable_name);

            return;
        } else {
            compile_expression(value, 8);
            fprintf(target, "pop.d [rdx - 0x%lx] ; writing to argument '", safe_zone_offset - i * 8);
    ...
    free((void*)function_name->variable_name);
}
    ...
    while(asm_length) {
        ...
        if(next == '%') {
            ...
            if(variable_name_length == 0) {
                linked_compiler->error_positioned(node->source_position, "empty variable reference in inline assembly");
            } else {
                inline_variable.variable_name = strndup(variable_name_start, variable_name_length);
                inline_variable.name_length = variable_name_length;
...
void backend_context::compile_bonk_statement(tree_node_operator* node, unsigned long stack_bytes_expected) {
...
void backend_context::compile_brek_statement(tree_node_operator* node, unsigned long stack_bytes_expected) {
    if(cycle_label_id == 0) {
        linked_compiler->error_positioned(node->source_position, "brek statement outside cycle");
    } else {
        if(stack_bytes_expected > 0) {
            linked_compiler->error_positioned(node->source_position, "brek does not return anything");
        }
        fprintf(target, "jmp _cycle_%lu_end\n", cycle_label_id);
    }
}

void backend_context::compile_rebonk_statement(tree_node_operator* node, unsigned long stack_bytes_expected) {
    if(cycle_label_id == 0) {
        linked_compiler->error_positioned(node->source_position, "rebonk statement outside cycle");
    } else {
        if(stack_bytes_expected > 0) {
            linked_compiler->error_positioned(node->source_position, "rebonk does not return anything");
        }
        fprintf(target, "jmp _cycle_%lu\n", cycle_label_id);
    }
}

void backend_context::compile_assignment(tree_node_identifier* variable, tree_node* value, unsigned long stack_bytes_expected) {
    compile_expression(value, 8);
    if(stack_bytes_expected != 0) {
...
void backend_context::compile_check(tree_node_check* node, unsigned long stack_bytes_expected) {
    ...
    if(comparsion->type == TREE_NODE_TYPE_OPERATOR) {
        comparsion_instruction = get_comparsion_instruction_negated(((tree_node_operator*)comparsion)->oper_type);
    }
    ...
    if(node->or_body) {
        ...
        fprintf(target, "jmp _if_%lu_end\n"
                                     "_if_%lu_else:\n", label_id, label_id);
...
void backend_context::compile_expression(tree_node* node, unsigned long stack_bytes_expected) {
    ...
    if(node->type == TREE_NODE_TYPE_OPERATOR) {
        ...
        switch(oper->oper_type) {
            ...
            case BONK_OPERATOR_BONK:       compile_bonk_statement  (oper, stack_bytes_expected);     break;
            case BONK_OPERATOR_BREK:       compile_brek_statement  (oper, stack_bytes_expected);     break;
            case BONK_OPERATOR_REBONK:     compile_rebonk_statement(oper, stack_bytes_expected);     break;
            case BONK_OPERATOR_ASSIGNMENT: compile_assignment      ((tree_node_identifier*)oper->left, oper->right, stack_bytes_expected);     break;
            case BONK_OPERATOR_OR:
            case BONK_OPERATOR_AND:        compile_logic           (oper, stack_bytes_expected);     break;
        ...
        if(var_definition->variable_value) compile_assignment(var_definition->variable_name, var_definition->variable_value, stack_bytes_expected);
        else return;
...
void backend_context::field_list_declare_variable(tree_node_variable_definition* node) {
    ...
    if(var == nullptr) {
        var = variable_number::create(node->variable_name);
...
field_list* backend_context::field_list_find_block_parameters(tree_node_block_definition* block) {
    field_list* argument_list = field_list::create();
    if(!argument_list) return nullptr;
    ...
    for(auto i = list->begin(); i != list->end(); list->next_iterator(&i)) {
        ...
        variable* number = variable_number::create(var_definition->variable_name);
...
field_list* backend_context::read_scope_variables(tree_node_list<tree_node*>* node, bool reset_frame_offset) {

    field_list* scope = field_list::create();
        ...
        for(auto i = list->begin(); i != list->end(); list->next_iterator(&i)) {
            ...
            if(next_node->type == TREE_NODE_TYPE_VAR_DEFINITION) {
                ...
                if(var_definition->variable_value) {
                    compile_assignment(var_definition->variable_name, var_definition->variable_value, 0);
                }
            }
        }
