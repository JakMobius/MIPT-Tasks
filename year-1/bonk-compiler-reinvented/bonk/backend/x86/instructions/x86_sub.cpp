...
sub_command* sub_command::create_reg64_reg64(abstract_register target, abstract_register source, command_list* list) {
    X86_COMMAND_ALLOC_CONSTRUCT(2, 2, 1, sub_command, list, construct_reg64_reg64, target, source)
}

sub_command* sub_command::create_reg64_imm32(abstract_register reg64, uint32_t imm32, command_list* list) {
    X86_COMMAND_ALLOC_CONSTRUCT(2, 1, 1, sub_command, list, construct_reg64_imm32, reg64, imm32)
}

void sub_command::construct_reg64_reg64(abstract_register target, abstract_register source) {
    type = COMMAND_SUB;
...
void sub_command::construct_reg64_imm32(abstract_register reg64, uint32_t imm32) {
    type = COMMAND_SUB;
...
void sub_command::to_bytes(command_encoder* buffer) {
    ...
    if(target.type == PARAMETER_TYPE_REG_64 && source.type == PARAMETER_TYPE_REG_64) {
        write_prefix_opcode_modrm_sib(buffer, 0x29, source, target);

        return;
    }

    if(target.type == PARAMETER_TYPE_REG_64 && source.type == PARAMETER_TYPE_IMM) {
        write_extended_opcode(buffer, 0x81, 0x05, target);
