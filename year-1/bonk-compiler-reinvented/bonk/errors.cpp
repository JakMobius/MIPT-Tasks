...
void compiler::warning_positioned(parser_position* pos, const char *format, ...) {
    if(config->error_file == nullptr) return;
    va_list ap;
...
void parser::warning(const char *format, ...) {
    if(linked_compiler->config->error_file == nullptr) return;
    va_list ap;

    va_start (ap, format);
    parser_position* pos = input->point((int)position)->position;
    fprintf (linked_compiler->config->error_file, "%s:%lu:%lu: warning: ", pos->filename, pos->line, pos->ch);
...
void parser::error(const char *format, ...) {
    ...
    parser_position* pos = input->point((int)position)->position;
    if(pos) {
        fprintf(linked_compiler->config->error_file, "%s:%lu:%lu: error: ", pos->filename, pos->line, pos->ch);
    } else {
...
void parser::fatal_error(const char *format, ...) {
    ...
    parser_position* pos = input->get((int)position).position;
