...
s_tree_node* bonk_parse_grammatic_unary_operator(s_bonk_parser* parser) {
    ...
    if(lexeme->type == BONK_LEXEME_KEYWORD) {
        ...
        e_bonk_operator_type operator = -1;

        if(lexeme->keyword_data.keyword_type == BONK_KEYWORD_BREK) {
            operator = BONK_OPERATOR_BREK;
        } else if(lexeme->keyword_data.keyword_type == BONK_KEYWORD_REBONK){
            operator = BONK_OPERATOR_REBONK;
        }

        if(operator != -1) {
            bonk_parser_eat_lexeme(parser);

            s_tree_node* statement = (s_tree_node*) tree_create_node_operator(BONK_OPERATOR_BREK);
            if(!statement) {
                compiler_out_of_memory(parser->compiler);
                return NULL;
            }

            statement->source_position = bonk_parser_position_clone(lexeme->position);
            return statement;
        }
