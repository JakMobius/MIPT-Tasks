...
s_mlist* mlist_new(mlist_index capacity) {
    s_mlist* list = (s_mlist*)calloc(1, sizeof(s_mlist));
...
e_mlist_error mlist::delete_index(mlist_index index, mlist_element_type* target) {
    ...
    if(target != NULL) *target = element_array[index];
...
e_mlist_error mlist::resize(mlist_index new_capacity) {
    ...
    mlist_element_type* element_array = (mlist_element_type*)realloc(this->element_array, new_capacity * sizeof(mlist_element_type));
    if(!element_array) return MLIST_ERROR_OUT_OF_MEMORY;
    this->element_array = element_array;
    
    mlist_index* next_indices = (mlist_index*)realloc(this->next_indices, new_capacity * sizeof(mlist_index));
    if(!next_indices) return MLIST_ERROR_OUT_OF_MEMORY;
    this->next_indices = next_indices;
    
    mlist_index* prev_indices = (mlist_index*)realloc(this->prev_indices, new_capacity * sizeof(mlist_index));
    if(!prev_indices) return MLIST_ERROR_OUT_OF_MEMORY;
    this->prev_indices = prev_indices;
    
    if(new_capacity > capacity) {
        
        mlist_index relinker = capacity;
        
        for(mlist_index index = relinker; index < new_capacity; index++) {
...
e_mlist_error mlist::linear_address_search(mlist_index index, mlist_index* target) {
