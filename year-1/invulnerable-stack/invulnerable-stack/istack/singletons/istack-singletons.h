...
const char* istack_err_get_description(istack_err_t err) {
    ...
    if(err == ISTACK_ERR_ZERO_CAPACITY) return ISTACK_ERR_T_DESCRIPTIONS[6];
    if(err == ISTACK_CORRUPT_HASH_CHECK_FAILED) return ISTACK_ERR_T_DESCRIPTIONS[7];
    if(err == ISTACK_CORRUPT_POISON_LEAK) return ISTACK_ERR_T_DESCRIPTIONS[8];
    if(err == ISTACK_CORRUPT_SIZE_GREATER_THAN_CAPACITY) return ISTACK_ERR_T_DESCRIPTIONS[9];
    if(err == ISTACK_CORRUPT_CLONE_INCONSISTENCY) return ISTACK_ERR_T_DESCRIPTIONS[10];
    if(err == ISTACK_CORRUPT_DESCRIPTOR_LIST_FAILURE) return ISTACK_ERR_T_DESCRIPTIONS[11];
    if(err == ISTACK_CORRUPT_INVALID_POINTER) return ISTACK_ERR_T_DESCRIPTIONS[12];
    return "((null))";
}
