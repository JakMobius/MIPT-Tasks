...
ColorPickerWindow::ColorPickerWindow(AppView* app, const Vec2f &position) : AppWindow(app, position, {400, 500}, "Color picker") {
    ...
    rgba_slider_styles[3]->set_bar_texture(Assets.canvas_background_texture.get());

    get_content_view()->append_child(container);
}

void ColorPickerWindow::set_color(const Vec4f &color) {
    current_rgb.m_content = color.content;
...
void ColorPickerWindow::update_rgba_slider_gradients() {
    for(int i = 0; i < 4; i++) {
        ...
        end_color->content = current_rgb.m_content;
        end_color->set_w(1);
        end_color->set(i, 1);

        start_color->content = current_rgb.m_content;
        start_color->set_w(1);
        start_color->set(i, 0);
...
void ColorPickerWindow::update_value_slider_gradient() {
    ...
    slider_color->content = max_valued_color.m_content;
...
void ColorPickerWindow::layout() {
    ...
    if(!first_layout) {
        first_layout = true;
        Vec4f color { current_rgb.m_content };
