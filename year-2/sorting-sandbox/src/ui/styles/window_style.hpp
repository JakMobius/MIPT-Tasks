...
struct UIDefaultCloseButtonStyle : public UIButtonStyle {
    ...
    const UIFillStyle* get_hovered_color() const override {
        ...
        if(!texture.get_texture()) texture.set_texture(Assets.window_close_hover_icon.get());
        return &texture;
    }
    const UIFillStyle* get_inactive_color()    const override {
        static UIFillStyleTexture texture {};
        if(!texture.get_texture()) texture.set_texture(Assets.window_close_inactive_icon.get());
        return &texture;
    }
    const UIFillStyle* get_idle_color()    const override {
        static UIFillStyleTexture texture {};
        if(!texture.get_texture()) texture.set_texture(Assets.window_close_icon.get());
        return &texture;
    }
    const UIFillStyle* get_clicked_color() const override {
        static UIFillStyleTexture texture {};
        if(!texture.get_texture()) texture.set_texture(Assets.window_close_clicked_icon.get());
        return &texture;
    }
};
