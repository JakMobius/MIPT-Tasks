...
void UIButton::update_state() {
    ...
    else if(!m_active) set_fill_style(style->get_inactive_color(), 0.3);
    else if(m_hovered) set_fill_style(style->get_hovered_color(), 0.3);
    else if(m_clicked) set_fill_style(style->get_clicked_color(), 0.3);
...
void UIButton::set_enabled(bool p_enabled) {
    if(!p_enabled && enabled && m_clicked) {
        m_transform.translate(0, -2);
    }
    enabled = p_enabled;
    update_state();
