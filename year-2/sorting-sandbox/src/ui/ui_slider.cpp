...
void UISlider::on_mouse_down(MouseDownEvent* event) {
    ...
    if(m_current_clicked_child != button) {
...
void UISlider::set_button_position(float x) {
    float available_width = m_size[0] - button->get_size()[0];
...
void UISlider::layout() {
    ...
    bar->set_size({m_size[0], bar->get_size()[1]});
    bar->set_position({0, button->get_size()[1] / 2 - style->get_bar_height() / 2});
    float position_x = (m_size[0] - button->get_size()[0]) * fraction;
...
void UISlider::set_style(const UISliderStyle* p_style) {
    ...
    set_size(m_size);
    bar->set_size({m_size[0], style->get_bar_height()});
}
