...
void UIWindowContainer::on_mouse_down(MouseDownEvent* event) {
    ...
    if(m_current_hovered_child != active_window) {
        if(m_current_hovered_child) {
            maybe_activate_child(m_current_hovered_child);
...
void UIWindowContainer::focus_window(UIWindow* window) {
    ...
    for(int i = view_index + 1; i < m_children.size(); i++) {
        m_children[i - 1] = m_children[i];
    }
    m_children[m_children.size() - 1] = active_window;
}
